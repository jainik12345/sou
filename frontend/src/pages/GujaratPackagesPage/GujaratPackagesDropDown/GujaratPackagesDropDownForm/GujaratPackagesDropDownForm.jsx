...
export const GujaratPackagesDropDownForm = () => {
  ...
  // 🔁 Mapping route string to gujarat_package_id
  const GujaratPackageMap = {
    "gujarat-tour-3n-4d": 1,
    "gujarat-tour-4n-5d": 2,
    "gujarat-tour-5n-6d": 3,
    "gujarat-tour-6n-7d": 4,
    "gujarat-tour-7n-8d": 5,
    "gujarat-tour-11n-12d": 6,
  };

  // ✅ Fetch heading based on gujarat_package_id
  useEffect(() => {
    const fetchPackageHeadings = async () => {
      const packageId = GujaratPackageMap[GujaratPath?.toLowerCase()];
      if (!packageId) return;
  ...
  if (!GujaratPackageMap[GujaratPath?.toLowerCase()]) {
    return (
      <h1 className="text-xl text-red-600 text-center p-5">
        Invalid Gujarat Package Path
      </h1>
    );
  }
  ...
  return (
    <div className="gujarat-packages-section p-10">
      ...
      <div className="form-cont max-w-screen-lg mx-auto md:p-10 p-5 white shadow-xl shadow-black/50">
        ...
        >
          {/* ... Name, Email, Phone, Date Fields as before ... */}
          ...
          >
            {PackageOptions.map((item, idx) => (
              <MenuItem key={idx} value={item.heading}>
                {item.heading}
              </MenuItem>
            ))}
          </TextField>

          {/* ... Message Field, Submit and Reset buttons ... */}
        </form>
      </div>
    </div>
  );
