...
const HomeNearAttractionsUpdate = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { rowData } = location.state || {};

  const [title, setTitle] = useState(rowData?.title || "");
  const [time, setTime] = useState(rowData?.time || "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`${BE_URL}/homeNearAttractions/${rowData.id}`, formData);
      setSuccess(true);
      setTimeout(() => {
        setSuccess(false);
            ...
            <div className="mt-2">
              ...
              <img
                src={
                  preview
                    ? preview
                    : `${BE_URL}/Images/HomeImages/HomeNearAttractions/${rowData.image}`
                }
                alt="Attraction"
...
export default HomeNearAttractionsUpdate;
