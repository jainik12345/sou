...
const HomeOnlineBookingContentTrace = () => {
  ...
  const fetchDeletedData = async () => {
    try {
      const res = await axios.get(`${BE_URL}/homeOnlineBookingContent/trashed`);
  ...
  const handleRestoreClick = async (id) => {
    try {
      await axios.patch(`${BE_URL}/homeOnlineBookingContent/restore/${id}`);
      setSelectedId(id);
    ...
    <div className="p-4 rounded-xl bg-white">
      ...
      <div className="flex justify-between mb-4">
        <h2 className="text-left font-semibold text-xl">
          Home Online Booking Content Trace
        </h2>
...
export default HomeOnlineBookingContentTrace;
