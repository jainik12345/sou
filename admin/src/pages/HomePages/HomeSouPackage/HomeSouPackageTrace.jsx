...
const HomeSouPackageTrace = () => {
  ...
  // Fetch trashed Home SOU Packages by package
  useEffect(() => {
    if (selectedPackageId) {
      axios
        .get(`${BE_URL}/homeSouPackage/trashed/package/${selectedPackageId}`)
  ...
  const handleRestore = (id) => {
    axios
      .patch(`${BE_URL}/homeSouPackage/restore/${id}`)
      .then((res) => {
    ...
    <div className="p-6 rounded-xl bg-white relative shadow-lg">
      ...
      >
        <h2 className="text-2xl font-semibold text-gray-900">
          Trashed Home SOU Package
        </h2>
        <Back onClick={() => navigate("/home-sou-package")} />
      </Box>
            ...
            {displayedRows.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} align="center" className="py-6">
                  First select package name, then trashed data will appear here.
                </TableCell>
              </TableRow>
            ) : (
              displayedRows.map((row, index) => (
                ...
                >
                  ...
                  <TableCell className="text-center">
                    <Tooltip title="Restore Home SOU Package" arrow>
                      <button
                        className="text-blue-600 cursor-pointer hover:text-blue-800"
                        onClick={() => handleRestore(row.id)}
                        aria-label="Restore Home SOU Package"
                      >
                        <FaRecycle size={22} />
                      </button>
                    </Tooltip>
...
export default HomeSouPackageTrace;
