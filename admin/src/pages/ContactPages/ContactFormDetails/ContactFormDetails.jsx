...
const ContactFormDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${BE_URL}/contactForm`);
  ...
  const handleReplySubmit = async () => {
    ...
    try {
      const response = await axios.post(`${BE_URL}/contactForm/reply`, {
  ...
  const handleDeleteRow = async (id) => {
    try {
      const response = await axios.delete(`${BE_URL}/contactForm/${id}`);
      if (response.data.status === "success") {
