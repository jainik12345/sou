...
const SOUPackageItineraryPackagePriceInsert = ({
  ...
  const navigate = useNavigate();
  const [formData, setFormData] = useState({
    sou_package_itinerary_id: initialData.sou_package_itinerary_id || "",
    image: null,
    package_start_price: initialData.package_start_price || "",
  ...
  const [otherPrices, setOtherPrices] = useState(
    initialData.other_price
      ? (() => {
          try {
            const parsed =
              typeof initialData.other_price === "string"
                ? JSON.parse(initialData.other_price)
                : initialData.other_price;
            return Array.isArray(parsed) && parsed.length > 0
              ? parsed
              : [defaultOtherPrice()];
          } catch {
            return [defaultOtherPrice()];
          }
        })()
      : [defaultOtherPrice()]
  );
  ...
  const handleSubmit = async (e) => {
    ...
    const newErrors = {
      sou_package_itinerary_id: formData.sou_package_itinerary_id === "",
      image: !formData.image && !isUpdate, // Only required for insert
      package_start_price: formData.package_start_price === "",
    ...
    try {
      ...
      if (isUpdate && initialData.image && !formData.image)
      ...
      if (isUpdate) {
        res = await axios.put(
          `${BE_URL}/souPackageItineraryPackagePrice/${initialData.id}`,
          data,
          {
            headers: {
              "Content-Type": "multipart/form-data",
      ...
      if (res.data.status === "success") {
        ...
        alert(isUpdate ? "Update failed" : "Insert failed");
      }
    } catch (error) {
      console.error(isUpdate ? "Update error:" : "Insert error:", error);
      alert("An error occurred");
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="border-2 border-blue-300 rounded-2xl p-6 shadow-md">
        <h2 className="bg-blue-600 text-white text-lg font-semibold py-3 px-5 rounded-md mb-8">
          {isUpdate ? "Update" : "Add"} SOU Package Itinerary Package Price
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            ...
            {/* Show existing image if in update mode and no new image */}
            {isUpdate && initialData.image && !formData.image && (
              <div className="mt-2">
                <img
                  src={`${BE_URL}/Images/SouPackage/SouPackageItineraryPackagePriceImages/${initialData.image}`}
                  alt="Current"
                  style={{ maxWidth: 180, borderRadius: 8 }}
                />
                <div className="text-xs text-gray-500 mt-1">
                  Current image will be kept if none is selected.
                </div>
              </div>
            )}
          </div>
