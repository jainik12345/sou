...
const SOUPackageItinerarySectionUpdate = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const itinerarySectionData = location.state?.itinerarySectionData;
  ...
  useEffect(() => {
    ...
    if (itinerarySectionData) {
      setFormData({
        sou_package_itinerary_id: itinerarySectionData.sou_package_itinerary_id,
        heading: itinerarySectionData.heading || "",
        evenst: (() => {
          try {
            const parsed =
              typeof itinerarySectionData.evenst === "string"
                ? JSON.parse(itinerarySectionData.evenst)
                : itinerarySectionData.evenst;
            if (Array.isArray(parsed) && parsed.length > 0) {
              return parsed.map((v) => ({ value: v }));
            }
            return [{ value: "" }];
          } catch {
            return [{ value: "" }];
        ...
        id: itinerarySectionData.id,
      });
    } else {
      navigate("/sou-package-itinerary-section");
    }
  }, [itinerarySectionData, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(
        `${BE_URL}/souPackageItinerarySection/${formData.id}`,
        data
      );
    ...
    <div className="p-6">
      <div className="border-2 border-blue-300 rounded-2xl p-6 shadow-md">
        <h2 className="bg-blue-600 text-white text-lg font-semibold py-3 px-5 rounded-md mb-8">
          Update SOU Package Itinerary Section
        </h2>
...
export default SOUPackageItinerarySectionUpdate;
