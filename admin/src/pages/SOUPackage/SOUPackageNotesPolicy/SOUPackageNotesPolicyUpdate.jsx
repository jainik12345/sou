...
const SOUPackageNotesPolicyUpdate = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const notesPolicyData = location.state?.notesPolicyData;
  ...
  useEffect(() => {
    ...
    if (notesPolicyData) {
      setFormData({
        sou_package_id: notesPolicyData.sou_package_id,
        notes: (() => {
          try {
            const parsed =
              typeof notesPolicyData.notes === "string"
                ? JSON.parse(notesPolicyData.notes)
                : notesPolicyData.notes;
        ...
        refund_policy: (() => {
          try {
            const parsed =
              typeof notesPolicyData.refund_policy === "string"
                ? JSON.parse(notesPolicyData.refund_policy)
                : notesPolicyData.refund_policy;
            if (Array.isArray(parsed) && parsed.length > 0) {
              return parsed.map((v) => ({ value: v }));
            }
            return [{ value: "" }];
        ...
        id: notesPolicyData.id,
      });
    } else {
      navigate("/sou-package-notes-policy");
    }
  }, [notesPolicyData, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(
        `${BE_URL}/souPackageNotesPolicy/${formData.id}`,
        data
      );
    ...
    <div className="p-6">
      <div className="border-2 border-blue-300 rounded-2xl p-6 shadow-md">
        <h2 className="bg-blue-600 text-white text-lg font-semibold py-3 px-5 rounded-md mb-8">
          Update SOU Package Notes & Policy
        </h2>
...
export default SOUPackageNotesPolicyUpdate;
