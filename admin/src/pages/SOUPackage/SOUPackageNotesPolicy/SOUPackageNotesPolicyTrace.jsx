...
const SOUPackageNotesPolicyTrace = () => {
  const [page, setPage] = useState(1);
  const rowsPerPage = 10;
  const [packageOptions, setPackageOptions] = useState([]);
  const [selectedPackageId, setSelectedPackageId] = useState("");
  const [trashedNotesPolicies, setTrashedNotesPolicies] = useState([]);
  ...
  // Fetch trashed notes/policies by package
  useEffect(() => {
    if (selectedPackageId) {
      axios
        .get(
          `${BE_URL}/souPackageNotesPolicy/trashed/package/${selectedPackageId}`
        )
        .then((res) => setTrashedNotesPolicies(res.data.data))
        .catch((err) => console.error("Trash fetch failed:", err));
    } else {
      setTrashedNotesPolicies([]);
    }
  }, [selectedPackageId]);
  ...
  const handleRestore = (id) => {
    axios
      .patch(`${BE_URL}/souPackageNotesPolicy/restore/${id}`)
      .then((res) => {
        if (res.data.status === "success") {
          setTrashedNotesPolicies((prev) =>
  ...
  const displayedRows = trashedNotesPolicies.slice(
    (page - 1) * rowsPerPage,
    page * rowsPerPage
  );
    ...
    <div className="p-6 rounded-xl bg-white relative shadow-lg">
      ...
      >
        <h2 className="text-2xl font-semibold text-gray-900">
          Trashed Package Notes & Policy
        </h2>
        <Back onClick={() => navigate("/sou-package-notes-policy")} />
      </Box>
      ...
      <TableContainer component={Paper} className="shadow-md">
            ...
            {displayedRows.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} align="center" className="py-6">
                  First select package name, then notes & policy data will
                  appear here.
                </TableCell>
              </TableRow>
            ) : (
              displayedRows.map((row, index) => (
                ...
                >
                  ...
                  <TableCell className="text-center">
                    <Tooltip title="Restore Notes & Policy" arrow>
                      <IconButton
                        className="text-blue-600 cursor-pointer hover:text-blue-800"
                        onClick={() => handleRestore(row.id)}
                        aria-label="Restore Notes & Policy"
                        size="small"
                      >
                        <FaRecycle size={22} />
                      </IconButton>
        ...
        <div className="flex justify-end p-4">
          <Pagination
            count={Math.ceil(trashedNotesPolicies.length / rowsPerPage)}
            page={page}
...
export default SOUPackageNotesPolicyTrace;
