...
const SOUPackageFAQSTrace = () => {
  const [page, setPage] = useState(1);
  const rowsPerPage = 10;
  const [packageOptions, setPackageOptions] = useState([]);
  const [selectedPackageId, setSelectedPackageId] = useState("");
  const [trashedFaqs, setTrashedFaqs] = useState([]);
  ...
  // Fetch trashed FAQs by package
  useEffect(() => {
    if (selectedPackageId) {
      axios
        .get(`${BE_URL}/souPackageFaqs/trashed/package/${selectedPackageId}`)
        .then((res) => setTrashedFaqs(res.data.data))
  ...
  const handleRestore = (id) => {
    axios
      .patch(`${BE_URL}/souPackageFaqs/restore/${id}`)
      .then((res) => {
        if (res.data.status === "success") {
          setTrashedFaqs((prev) => prev.filter((item) => item.id !== id));
  ...
  const displayedRows = trashedFaqs.slice(
    (page - 1) * rowsPerPage,
    page * rowsPerPage
  );
    ...
    <div className="p-6 rounded-xl bg-white relative shadow-lg">
      ...
      >
        <h2 className="text-2xl font-semibold text-gray-900">
          Trashed Package FAQS
        </h2>
        <Back onClick={() => navigate("/sou-package-faqs")} />
      </Box>
      ...
      <TableContainer component={Paper} className="shadow-md">
            ...
            {displayedRows.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} align="center" className="py-6">
                  First select package name, then FAQ data will appear here.
                </TableCell>
              </TableRow>
            ) : (
              displayedRows.map((row, index) => (
                ...
                >
                  ...
                  <TableCell className="text-center">
                    <Tooltip title="Restore FAQ" arrow>
                      <IconButton
                        className="text-blue-600 cursor-pointer hover:text-blue-800"
                        onClick={() => handleRestore(row.id)}
                        aria-label="Restore FAQ"
                        size="small"
                      >
                        <FaRecycle size={22} />
                      </IconButton>
        ...
        <div className="flex justify-end p-4">
          <Pagination
            count={Math.ceil(trashedFaqs.length / rowsPerPage)}
            page={page}
...
export default SOUPackageFAQSTrace;
