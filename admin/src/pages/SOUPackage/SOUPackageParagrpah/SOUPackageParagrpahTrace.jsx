...
const SOUPackageParagrpahTrace = () => {
  const [page, setPage] = useState(1);
  const rowsPerPage = 10;
  const [packageOptions, setPackageOptions] = useState([]);
  const [selectedPackageId, setSelectedPackageId] = useState("");
  const [trashedParagraphs, setTrashedParagraphs] = useState([]);
  ...
  // Fetch trashed paragraphs by package
  useEffect(() => {
    if (selectedPackageId) {
      axios
        .get(
          `${BE_URL}/souPackageParagraph/trashed/package/${selectedPackageId}`
        )
        .then((res) => setTrashedParagraphs(res.data.data))
        .catch((err) => console.error("Trash fetch failed:", err));
    } else {
      setTrashedParagraphs([]);
    }
  }, [selectedPackageId]);
  ...
  const handleRestore = (id) => {
    axios
      .patch(`${BE_URL}/souPackageParagraph/restore/${id}`)
      .then((res) => {
        if (res.data.status === "success") {
          setTrashedParagraphs((prev) => prev.filter((item) => item.id !== id));
  ...
  const displayedRows = trashedParagraphs.slice(
    (page - 1) * rowsPerPage,
    page * rowsPerPage
  );
    ...
    <div className="p-6 rounded-xl bg-white relative shadow-lg">
      ...
      >
        <h2 className="text-2xl font-semibold text-gray-900">
          Trashed Package Paragraph
        </h2>
        <Back onClick={() => navigate("/sou-package-paragrpah")} />
      </Box>
      ...
      <TableContainer component={Paper} className="shadow-md">
            ...
            {displayedRows.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} align="center" className="py-6">
                  First select package name, then paragraph data will appear
                  here.
                </TableCell>
              </TableRow>
            ) : (
              displayedRows.map((row, index) => (
                ...
                >
                  ...
                  <TableCell className="text-center">
                    <Tooltip title="Restore Paragraph" arrow>
                      <IconButton
                        className="text-blue-600 cursor-pointer hover:text-blue-800"
                        onClick={() => handleRestore(row.id)}
                        aria-label="Restore Paragraph"
                        size="small"
                      >
                        <FaRecycle size={22} />
                      </IconButton>
        ...
        <div className="flex justify-end p-4">
          <Pagination
            count={Math.ceil(trashedParagraphs.length / rowsPerPage)}
            page={page}
...
export default SOUPackageParagrpahTrace;
