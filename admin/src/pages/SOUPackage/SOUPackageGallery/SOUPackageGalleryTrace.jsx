...
const SOUPackageGalleryTrace = () => {
  ...
  const [trashedGalleries, setTrashedGalleries] = useState([]);
  ...
  // Fetch trashed galleries by package
  useEffect(() => {
    if (selectedPackageId) {
      axios
        .get(`${BE_URL}/souPackageGallery/trashed/package/${selectedPackageId}`)
        .then((res) => setTrashedGalleries(res.data.data))
  ...
  const handleRestore = (id) => {
    axios
      .patch(`${BE_URL}/souPackageGallery/restore/${id}`)
      .then((res) => {
        if (res.data.status === "success") {
          setTrashedGalleries((prev) => prev.filter((item) => item.id !== id));
  ...
  const displayedRows = trashedGalleries.slice(
    (page - 1) * rowsPerPage,
    page * rowsPerPage
  );
    ...
    <div className="p-6 rounded-xl bg-white relative shadow-lg">
      ...
      >
        <h2 className="text-2xl font-semibold text-gray-900">
          Trashed Package Gallery
        </h2>
        <Back onClick={() => navigate("/sou-package-gallery")} />
      </Box>
      ...
      <TableContainer component={Paper} className="shadow-md">
            ...
            {displayedRows.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} align="center" className="py-6">
                  First select package name, then gallery data will appear here.
                </TableCell>
              </TableRow>
            ) : (
              displayedRows.map((row, index) => (
                ...
                >
                  ...
                  <TableCell className="text-center">
                    <Tooltip title="Restore Gallery" arrow>
                      <button
                        className="text-blue-600 cursor-pointer hover:text-blue-800"
                        onClick={() => handleRestore(row.id)}
                        aria-label="Restore Gallery"
                      >
                        <FaRecycle size={22} />
                      </button>
                    </Tooltip>
        ...
        <div className="flex justify-end p-4">
          <Pagination
            count={Math.ceil(trashedGalleries.length / rowsPerPage)}
            page={page}
