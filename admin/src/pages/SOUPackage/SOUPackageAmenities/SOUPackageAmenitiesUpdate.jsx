...
const SOUPackageAmenitiesUpdate = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const amenityData = location.state?.amenityData;
  ...
  useEffect(() => {
    ...
    if (amenityData) {
      setFormData({
        sou_package_id: amenityData.sou_package_id,
        title: amenityData.title || "",
        id: amenityData.id,
      });
      setPreviewUrl(
        amenityData.image
          ? `${BE_URL}/Images/SouPackage/SouPackageAmenities/${amenityData.image}`
          : ""
      );
    } else {
      navigate("/sou-package-amenities");
    }
  }, [amenityData, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(
        `${BE_URL}/souPackageAmenities/${formData.id}`,
        data,
        {
...
export default SOUPackageAmenitiesUpdate;
