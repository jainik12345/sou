...
const SOUPackageAmenities = () => {
  ...
  // Fetch Amenities by selected package
  useEffect(() => {
    if (selectedPackageId) {
      axios
        .get(`${BE_URL}/souPackageAmenities/package/${selectedPackageId}`)
  ...
  const handleDelete = (id) => {
    axios
      .delete(`${BE_URL}/souPackageAmenities/${id}`)
      .then((res) => {
  ...
  const displayedRows = amenities.slice(
    (page - 1) * rowsPerPage,
    page * rowsPerPage
  );

  const handleAdd = () => navigate("/sou-package-amenities/insert");

  const handleUpdate = (item) => {
    navigate("/sou-package-amenities/update", {
      state: { amenityData: item },
    });
  };

  const handleTraceClick = () => navigate("/sou-package-amenities/trace");
...
export default SOUPackageAmenities;