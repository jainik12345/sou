...
const SOUPackageBookLayerUpdate = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const bookLayerData = location.state?.packageBookLayerData;
  ...
  useEffect(() => {
    ...
    if (bookLayerData) {
      setFormData({
        sou_package_id: bookLayerData.sou_package_id,
        title: bookLayerData.title || "",
        id: bookLayerData.id,
      });
    } else {
      navigate("/sou-package-book-layer");
    }
  }, [bookLayerData, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(
        `${BE_URL}/souPackageBookLayer/${formData.id}`,
        data
      );
...
export default SOUPackageBookLayerUpdate;
