...
const SOUPackageItineraryNameTrace = () => {
  const [page, setPage] = useState(1);
  const rowsPerPage = 10;
  const [packageOptions, setPackageOptions] = useState([]);
  const [selectedPackageId, setSelectedPackageId] = useState("");
  const [trashedItineraries, setTrashedItineraries] = useState([]);
  ...
  // Fetch trashed itinerary names by package
  useEffect(() => {
    if (selectedPackageId) {
      axios
        .get(
          `${BE_URL}/souPackageItineraryName/trashed/package/${selectedPackageId}`
        )
        .then((res) => setTrashedItineraries(res.data.data))
        .catch((err) => console.error("Trash fetch failed:", err));
    } else {
      setTrashedItineraries([]);
    }
  }, [selectedPackageId]);
  ...
  const handleRestore = (id) => {
    axios
      .patch(`${BE_URL}/souPackageItineraryName/restore/${id}`)
      .then((res) => {
        if (res.data.status === "success") {
          setTrashedItineraries((prev) =>
  ...
  const displayedRows = trashedItineraries.slice(
    (page - 1) * rowsPerPage,
    page * rowsPerPage
  );
    ...
    <div className="p-6 rounded-xl bg-white relative shadow-lg">
      ...
      >
        <h2 className="text-2xl font-semibold text-gray-900">
          Trashed Package Itinerary Name
        </h2>
        <Back onClick={() => navigate("/sou-package-itinerary-name")} />
      </Box>
      ...
      <TableContainer component={Paper} className="shadow-md">
            ...
            {displayedRows.length === 0 ? (
              <TableRow>
                <TableCell colSpan={3} align="center" className="py-6">
                  First select package name, then itinerary name data will
                  appear here.
                </TableCell>
              </TableRow>
            ) : (
              displayedRows.map((row, index) => (
                ...
                >
                  ...
                  <TableCell className="text-center">
                    <Tooltip title="Restore Itinerary Name" arrow>
                      <button
                        className="text-blue-600 cursor-pointer hover:text-blue-800"
                        onClick={() => handleRestore(row.id)}
                        aria-label="Restore Itinerary Name"
                      >
                        <FaRecycle size={22} />
                      </button>
                    </Tooltip>
        ...
        <div className="flex justify-end p-4">
          <Pagination
            count={Math.ceil(trashedItineraries.length / rowsPerPage)}
            page={page}
...
export default SOUPackageItineraryNameTrace;
