...
const SOUPackageItineraryNameUpdate = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const itineraryNameData = location.state?.itineraryNameData;
  ...
  const [errors, setErrors] = useState({
    ...
    sou_package_itinerary_name: false,
  });
  ...
  useEffect(() => {
    ...
    if (itineraryNameData) {
      setFormData({
        sou_package_id: itineraryNameData.sou_package_id,
        sou_package_itinerary_name:
          itineraryNameData.sou_package_itinerary_name || "",
        id: itineraryNameData.id,
      });
    } else {
      navigate("/sou-package-itinerary-name");
    }
  }, [itineraryNameData, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(
        `${BE_URL}/souPackageItineraryName/${formData.id}`,
        data
      );
    ...
    <div className="p-6">
      <div className="border-2 border-blue-300 rounded-2xl p-6 shadow-md">
        <h2 className="bg-blue-600 text-white text-lg font-semibold py-3 px-5 rounded-md mb-8">
          Update SOU Package Itinerary Name
        </h2>
...
export default SOUPackageItineraryNameUpdate;
