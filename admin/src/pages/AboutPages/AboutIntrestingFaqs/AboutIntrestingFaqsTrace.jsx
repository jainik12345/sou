...
const AboutIntrestingFaqsTrace = () => {
  ...
  const fetchTrashedData = async () => {
    try {
      const res = await axios.get(`${BE_URL}/aboutIntrestingFaqs/trashed`);
      setData(res.data.data || []);
    } catch (err) {
      console.error("Error fetching trashed FAQs:", err);
    }
  };

  const handleRestore = async (id) => {
    try {
      await axios.patch(`${BE_URL}/aboutIntrestingFaqs/restore/${id}`);
          ...
          <TableBody>
            {displayedRows.map((row, index) => (
              ...
              >
                ...
                <TableCell className="border-r">{row.questions}</TableCell>
                <TableCell className="border-r">{row.answer}</TableCell>
                <TableCell>
                  <button
