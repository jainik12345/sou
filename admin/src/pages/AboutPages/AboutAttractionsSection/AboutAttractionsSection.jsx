...
const AboutAttractionsSection = () => {
  ...
  const fetchAttractions = async () => {
    try {
      const res = await axios.get(`${BE_URL}/aboutAttractionsSection`);
      const formatted = res.data.data.map((item) => ({
        ...item,
        imageUrl: `${BE_URL}/Images/AboutImages/Attractions/${item.image}`,
      }));
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${BE_URL}/aboutAttractionsSection/${id}`);
  ...
  return (
    <div className="p-4 rounded-xl bg-white">
      ...
      <div className="flex justify-between mb-4">
        <Trace onClick={() => navigate("/about-attractions-section/trace")} />
        <Add
          text="Add Attraction"
          width="w-[200px]"
          onClick={() => navigate("/about-attractions-section/insert")}
        />
      </div>
              ...
              >
                ...
                <TableCell className="text-left">
                  <div className="flex space-x-4">
                    <IconButton
                      onClick={() =>
                        navigate("/about-attractions-section/update", {
                          state: { rowData: row },
                        })
                      }
                    >
...
export default AboutAttractionsSection;
