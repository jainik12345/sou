...
const AboutWhyChooseSectionTrace = () => {
  ...
  const fetchTrashedData = async () => {
    try {
      const res = await axios.get(`${BE_URL}/aboutWhyChooseSection/trashed`);
      setData(res.data.data || []);
    } catch (err) {
      console.error("Error fetching trashed Why Choose sections:", err);
    }
  };

  const handleRestore = async (id) => {
    try {
      await axios.patch(`${BE_URL}/aboutWhyChooseSection/restore/${id}`);
    ...
    <div className="p-4 rounded-xl bg-white">
      ...
      <div className="flex justify-between mb-4">
        <h2 className="text-left font-semibold text-xl">
          About Why Choose Section Trace
        </h2>
