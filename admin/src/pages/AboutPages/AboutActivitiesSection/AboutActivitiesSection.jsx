...
const AboutActivitiesSection = () => {
  ...
  const fetchActivities = async () => {
    try {
      const res = await axios.get(`${BE_URL}/aboutActivitiesSection`);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${BE_URL}/aboutActivitiesSection/${id}`);
  ...
  const displayedRows = activities.slice(
    (page - 1) * rowsPerPage,
    page * rowsPerPage
  );
  ...
  return (
    <div className="p-4 rounded-xl bg-white">
      ...
      <div className="flex justify-between mb-4">
        <Trace onClick={() => navigate("/about-activities-section/trace")} />
        <Add
          text="Add Activity"
          width="w-[200px]"
          onClick={() => navigate("/about-activities-section/insert")}
        />
      </div>
              ...
              >
                ...
                <TableCell className="text-left">
                  <div className="flex space-x-4">
                    <IconButton
                      onClick={() =>
                        navigate("/about-activities-section/update", {
                          state: { rowData: row },
                        })
                      }
                    >
